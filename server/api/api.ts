...
- Add limit to body length for incoming requests
*/
...
const HEADERS = {
	...
	'Access-Control-Allow-Methods': 'PUT, PATCH, DELETE, GET, POST, HEAD, OPTIONS',
...
export class APIResponse {
    ...
    public send(data: any, code: number = 200) {
        ...
        if (data !== null) this.rep.write(JSON.stringify(data));
        this.rep.end();
    }
}
...
export class API {
    ...
    public head(path: string) {
        this.on("OPTIONS", path, async (req: APIRequest, rep: APIResponse) => {rep.send(null);});
        this.on("HEAD", path, async (req: APIRequest, rep: APIResponse) => {rep.send(null);});
    }
    ...
    public put(path: string, cb: APICallback) {
        this.on("OPTIONS", path, async (req: APIRequest, rep: APIResponse) => {rep.send(null);});
        this.on("PUT", path, cb);
    }

    public patch(path: string, cb: APICallback) {
        this.on("OPTIONS", path, async (req: APIRequest, rep: APIResponse) => {rep.send(null);});
        this.on("PATCH", path, cb);
    }

    public delete(path: string, cb: APICallback) {
        this.on("OPTIONS", path, async (req: APIRequest, rep: APIResponse) => {rep.send(null);});
